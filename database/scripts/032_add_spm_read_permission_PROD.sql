-- 032_add_spm_read_permission_PROD.sql
--
...
--
-- Target: the LIVE catalog. Prod-asserting twin of 032_add_spm_read_permission.sql;
-- the body below is byte-identical, only the assertion differs. CONFIRM_PROD_WRITE
-- Apply with `sqlcmd -f 65001 -C`.

IF DB_NAME() <> N'KSS_Auth_Prod' RAISERROR('WRONG DATABASE', 20, 1) WITH LOG;
